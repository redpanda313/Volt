# Volt jam — night8 carrier drones + powerups
...
| `powerups_sheet_labeled.png` | sheet | Labeled reference atlas — excluded from Web export. |

API: `Art.night8_powerup_tex(kind)`, `Art.powerup_tex(kind)`, `Art.has_night8_powerups()`, `Art.placeholder_powerup_tex(kind)`.

The cargo sprite hangs under the drone. Collect on contact or tap (see `PLAYTEST.md`).
